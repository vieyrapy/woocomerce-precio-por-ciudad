...
function precios_ciudad_jquery_script() {
    ?>
    <script>
        jQuery(document).ready(function($) {
            // Obtener el campo de selección de ciudad
            var ciudadSelect = $('#ciudad_producto');

            // Obtener todos los campos de entrada y etiquetas de ciudad
            var camposCiudad = $('p.form-field[class*="precio_adicional"]');
            var etiquetasCiudad = $('span[class*="precio-ciudad-label"]');

            // Mostrar u ocultar campos de entrada y etiquetas de ciudad según la selección
            ciudadSelect.change(function() {
                var selectedCiudad = $(this).val();
                camposCiudad.hide();
                etiquetasCiudad.hide();

                // Mostrar solo el campo y la etiqueta de la ciudad seleccionada, si tiene un precio cargado
                if (selectedCiudad) {
                    var campoCiudadSeleccionada = $('p.form-field.precio_adicional\\[' + selectedCiudad + '\\]_field');
                    if (campoCiudadSeleccionada.find('.campo-ciudad').val() !== "") {
                        campoCiudadSeleccionada.show();
                        campoCiudadSeleccionada.next('span.precio-ciudad-label').show();
                    }
                }
            });

            // Ejecutar el cambio al cargar la página
            ciudadSelect.trigger('change');
        });
    </script>
    <?php
}
add_action('admin_footer', 'precios_ciudad_jquery_script');
